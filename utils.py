...
class DTADataset(InMemoryDataset):
    def __init__(self, root: str, dataset_name: str, drugs: np.ndarray,
                 y: np.ndarray, target_key: np.ndarray,
    ...
    def process(self, drugs: np.ndarray, target_key: np.ndarray, y: np.ndarray,
                smile_graph_dict: dict[str, Graph], target_graph_dict: dict[str, Graph]):
        assert (len(drugs) == len(target_key) and len(drugs) == len(y)), 'The three lists must be the same length!'
        data_list_mol = []
        data_list_pro = []
        data_len = len(drugs)
...
def _convert(graph: Graph, labels) -> DATA.Data:
    GCNData = DATA.Data(x=torch.Tensor(graph.features),
                            edge_index=torch.LongTensor(
                            graph.edge_index).transpose(1, 0),
...
def train(model: GNNNet, device: device, train_loader: DataLoader, optimizer: Adam, epoch: int):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if idx % LOG_INTERVAL == 0:
            print(f'Train epoch: {epoch} [{idx*BATCH_SIZE}/{data_len} ({100*idx/len(train_loader)}%)]\tLoss: {loss.item()}')
...
def create_bin_file(file_path: Path, data: Any) -> None:
    create_folder(file_path.parent)
