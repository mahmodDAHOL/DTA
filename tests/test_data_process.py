...
def data():
    ...
    aln_file = data_path.joinpath("aln/MLK1.aln")
    contact_file = data_path.joinpath("pconsc4/MLK1.npy")
...
def test_target_to_feature(data):
    ...
    assert feature.shape == (373, 54)
    
def test_target_to_graph(data):
    result = target_to_graph(data["data_path"], data["target_key"], data["target_sequence"])
    assert isinstance(result, Graph)
    assert result.size == np.array(373)
    assert result.edge_index.shape == (1859, 2)
    assert result.features.shape == (373, 54)
